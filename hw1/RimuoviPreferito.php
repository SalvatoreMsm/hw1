<?php
    ...
    $DeleteQuery = "DELETE FROM `PREFERITO` WHERE ID_ARTICOLO = '$id' AND TITLE = '$titolo' AND  USERNAME = '$username'";

    $res = mysqli_query($conn, $ControlQuery) or die("Errore: ".mysqli_error($conn));

    if(mysqli_num_rows($res)!=0){
        mysqli_query($conn, $DeleteQuery) or die("Errore: ".mysqli_error($conn));  
        echo 'Article removed';
    }
    else{
        echo "Trying to remove a not liked article";
        exit;
    }
